...
{
    public class InsertEventEndTimeCommandHandler : IRequestHandler<InsertEventEndTimeCommand, RequestResult<bool>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IEventRepository _eventRepository;
        private readonly IGameRepository _gameRepository;

        public InsertEventEndTimeCommandHandler(
        ...
        public async Task<RequestResult<bool>> Handle(InsertEventEndTimeCommand request, CancellationToken cancellationToken)
        {
            var game = await _gameRepository.GetById(request.IdGame);

            var eve = new Event(game, request.EventType);
            eve.UpdateEndTime(request.EndTime);
