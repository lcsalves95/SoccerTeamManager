...
{
    public class InsertEventWarningCommandHandler : IRequestHandler<InsertEventWarningCommand, RequestResult<bool>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IEventRepository _eventRepository;
        private readonly IGameRepository _gameRepository;

        public InsertEventWarningCommandHandler(
        ...
        public async Task<RequestResult<bool>> Handle(InsertEventWarningCommand request, CancellationToken cancellationToken)
        {
            var game = await _gameRepository.GetById(request.IdGame);

            var eve = new Event(game, request.EventType);
            eve.UpdateWarning(request.Warning);
