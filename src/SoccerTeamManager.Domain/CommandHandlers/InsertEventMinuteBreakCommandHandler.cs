...
{
    public class InsertEventMinuteBreakCommandHandler : IRequestHandler<InsertEventMinuteBreakCommand, RequestResult<bool>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IEventRepository _eventRepository;
        private readonly IGameRepository _gameRepository;

        public InsertEventMinuteBreakCommandHandler(
        ...
        public async Task<RequestResult<bool>> Handle(InsertEventMinuteBreakCommand request, CancellationToken cancellationToken)
        {
            var game = await _gameRepository.GetById(request.IdGame);

            var eve = new Event(game, request.EventType);
            eve.UpdateMinuteBreak(request.MinuteBreak);
