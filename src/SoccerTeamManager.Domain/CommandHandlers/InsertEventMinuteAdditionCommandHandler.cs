...
{
    public class InsertEventMinuteAdditionCommandHandler : IRequestHandler<InsertEventMinuteAdditionCommand, RequestResult<bool>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IEventRepository _eventRepository;
        private readonly IGameRepository _gameRepository;

        public InsertEventMinuteAdditionCommandHandler(
        ...
        public async Task<RequestResult<bool>> Handle(InsertEventMinuteAdditionCommand request, CancellationToken cancellationToken)
        {
            var game = await _gameRepository.GetById(request.IdGame);

            var eve = new Event(game, request.EventType);
            eve.UpdateMinuteAddition(request.MinuteAddition);
